...
# Decision tree classification is a non-linear and non-parametric algorithm. 
# It is a discriminative classifier that finds the hyperplane that best separates the classes.
# Decision tree classification is a type of instance-based learning, or lazy learning, where the function is only approximated locally and all computation is deferred until function evaluation.
# Decision tree classification is a simple algorithm that stores all available cases and classifies new cases based on a similarity measure (e.g., distance functions).
...
classifier = DecisionTreeClassifier(criterion = 'entropy', random_state = 0) # Create an object of the DecisionTreeClassifier class. The criterion parameter specifies the criterion to use for the split. The random_state parameter is used to ensure that we get the same results every time we run the code.
