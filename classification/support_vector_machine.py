...
# SVM is a non-linear and non-parametric algorithm. It is a discriminative classifier that finds the hyperplane that best separates the classes.
# SVM is a type of instance-based learning, or lazy learning, where the function is only approximated locally and all computation is deferred until function evaluation.
# SVM is a simple algorithm that stores all available cases and classifies new cases based on a similarity measure (e.g., distance functions).
...
classifier = SVC(kernel = 'linear', random_state = 0) # Create an object of the SVC class. The kernel parameter specifies the kernel type to be used in the algorithm. The random_state parameter is used to ensure that we get the same results every time we run the code.
