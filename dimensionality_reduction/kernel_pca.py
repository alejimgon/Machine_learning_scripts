# Kernel PCA
# Kernel PCA is a non-linear version of PCA. It is useful when the data is not linearly separable. 
# Kernel PCA is used to reduce the dimensionality of the data while preserving as much variance as possible. 
# Kernel PCA is used to identify the most important features in a dataset. 
# Kernel PCA is used to reduce the computational complexity of a machine learning model.
...
# Applying Kernel PCA
# We need to apply Kernel PCA before training the machine learning model because we want to reduce the dimensionality of the dataset.
from sklearn.decomposition import KernelPCA
kpca = KernelPCA(n_components = 2, kernel='rbf') # Create an object of the PCA class and specify the number of components we want to keep. We want to keep the two most important components.
X_train = kpca.fit_transform(X_train) # Fit and transfom the training set.
X_test = kpca.transform(X_test) # We only apply the transform method to the test set. We don't need to fit the test set because the LDA object is already fitted to the training set.
